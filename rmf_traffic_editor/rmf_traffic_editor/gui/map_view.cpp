...
// Google Maps Session 토큰 요청 함수
QString MapView::fetchSessionToken(const QString& api_key)
{
  // 세션 토큰 요청 URL
  QUrl url(QString("https://tile.googleapis.com/v1/createSession?key=%1").arg(api_key));
  qDebug() << "Generated URL:" << url.toString();

  // 요청 객체 생성
  QNetworkAccessManager local_network;
  QNetworkRequest request(url);
  request.setRawHeader("Content-Type", "application/json");

  // JSON 데이터 준비
  QJsonObject json_data;
  json_data["mapType"] = "satellite";
  json_data["language"] = "en-US";
  json_data["region"] = "US";

  // POST 요청 전송
  QByteArray payload = QJsonDocument(json_data).toJson();
  QNetworkReply* reply = local_network.post(request, payload);
  ...
  if (reply->error() == QNetworkReply::NoError)
  {
    ...
    if (response_doc.isObject())
    {
      ...
      if (response_obj.contains("session"))
      {
        delete reply;
        return response_obj["session"].toString();
      }
    }
  }

  // 오류 로그 출력
  qWarning("Failed to fetch session token: %s", reply->errorString().toUtf8().constData());
  qWarning() << "Error code:" << reply->errorString();
...
void MapView::request_tile(const int zoom, const int x, const int y)
{
  ...
  if (s_num_requests <= 2000)
  {
    ...
    // API 키 설정
    api_key = "AIzaSyBqArvCal3QID9-Zyu-WiYsBORdoKc6PjQ";

    // 세션 토큰 가져오기
    session_token = fetchSessionToken(api_key);
    if (session_token.isEmpty())
    {
      qWarning() << "Failed to fetch session token!";
      return;
    }
...
void MapView::request_finished(QNetworkReply* reply)
{
  ...
  */
  ...
  qDebug() << "Processing URL:" << QString::fromStdString(url);

  // Google Maps 타일 서버에 사용되는 Url 구성에 맞게 zoom, x, y 값 추출
  const size_t base_start = url.find("/2dtiles/");
  if (base_start == std::string::npos)
  {
    qWarning() << "Invalid URL format, missing '/2dtiles/'";
    return;
  }

  const size_t zoom_start = base_start + 9; // "/2dtiles/" 이후 시작
  const size_t zoom_end = url.find('/', zoom_start);
  printf("zoom_start: %zu\n", zoom_start);  // zoom_start 확인
  printf("zoom_end: %zu\n", zoom_end);      // zoom_end 확인
  if (zoom_end == std::string::npos)
  {
    qWarning() << "Invalid URL format, missing zoom value.";
    return;
  }
  const std::string zoom_str = url.substr(zoom_start, zoom_end - zoom_start);
  printf("zoom_str: %s\n", zoom_str.c_str());
  ...
  {
    qWarning() << "Invalid URL format, missing x value.";
    return;
  }
  const std::string x_str = url.substr(x_start, x_end - x_start);
  printf("x_str: %s\n", x_str.c_str());

  const size_t y_start = x_end + 1;
  const size_t y_end = url.find('s', 0);
  printf("y_start: %zu\n", y_start);  // y_start 확인
  printf("y_end: %zu\n", y_end);      // y_end 확인
  if (y_end == std::string::npos)
  {
    qWarning() << "Invalid URL format, missing y value.";
    return;
  }
  const std::string y_str = url.substr(y_start, y_end - y_start);
  printf("y_str: %s\n", y_str.c_str());
...
void MapView::process_request_queue()
{
  ...
  for (auto& tile : tile_pixmap_items)
  {
    ...
    {
      printf("request tile start\n");
      request_tile(tile.zoom, tile.x, tile.y);
      printf("request tile finished\n");
