...
def main(argv=sys.argv):
    ...
    robots_config = config_yaml['rmf_fleet']['robots']

    update_period = 1.0/fleet_mgr_yaml.get(
        'robot_state_update_frequency', 10.0
    )
    info(f'Robot state update frequency: {update_period} Sec')
    # api_prefix = (
    #     'http://' + fleet_mgr_yaml['ip'] + ':'
    #     + str(fleet_mgr_yaml['port'])
    # )
    # info(f'Fleet manager API prefix: {api_prefix}')
    # api = RobotAPI(
    #     api_prefix,
    #     fleet_mgr_yaml['user'],
    ...
    for robot_name in fleet_config.known_robots:
        # Validate robot name in robots_config
        if robot_name not in robots_config:
            raise ValueError(f"Robot '{robot_name}' configuration not found in 'robots_config'.")

        # Retrieve API details with fallback to fleet_manager defaults
        robot_api_ip = robots_config.get(robot_name, {}).get('ip', fleet_mgr_yaml['ip'])
    ...
    def update(self, state, data: RobotUpdateData):
        ...
        if self.execution:
            if data.is_command_completed(self.cmd_id):
                ...
                self.execution = None
                self.teleoperation = None
                info(f'Robot [{self.name}] has completed its current activity')
            else:
